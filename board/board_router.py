...
@router.get(path="/update/{post_no}", description="기본 게시판 - 특정 게시글 상세 조회")
async def read_post(post_no: int, db: Session = Depends(get_db)):
    return board_crud.get_post(post_no, db)
    
@router.put(path="/update/{post_no}", description="기본 게시판 - 특정 게시글 수정")
...
@router.patch(path="/delete/{post_no}", description="기본 게시판 - 특정 게시글 삭제")
async def delete_post_yn(post_no: int, db: Session = Depends(get_db)):
 return board_crud.alter_del_yn(post_no, db)


@router.delete(path="/delete/{post_no}", description="기본 게시판 - 특정 게시글 삭제")
