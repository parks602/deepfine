...
def get_post(post_no: int, db : Session):
    try:
        ...
        return Post(no=post.no, wrriter=post.writer, title=post.title, content=post.content, date=post.date)
    except Exception as e:
        return {'error' : str(e), 'msg': '게시글 번호가 존재하지 않습니다.'}
...
def update_post(update_post: UpdatePost, db: Session):
  ...
  try:
    if not post:
      raise Exception("게시글 번호가 존재하지 않습니다.")
